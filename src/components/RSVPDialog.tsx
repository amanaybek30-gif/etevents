...
const RSVPDialog = ({
  ...
  type FileAnswer = { name: string; url: string; size: number; type: string };
  type AnswerValue = string | string[] | FileAnswer[];
  const [customAnswers, setCustomAnswers] = useState<Record<string, AnswerValue>>({});
  const [uploadingQuestionId, setUploadingQuestionId] = useState<string | null>(null);
  const ATTENDEE_FILE_MAX_SIZE = 90 * 1024 * 1024;
  ...
  const updateCustomAnswer = (questionId: string, value: AnswerValue) => {
    setCustomAnswers(prev => ({ ...prev, [questionId]: value }));
  };

  const handleFileUploadForQuestion = async (q: CustomQuestion, files: FileList | null) => {
    if (!files || files.length === 0) return;
    const maxFiles = Math.max(1, Math.min(10, q.maxFiles || 1));
    const existing = (Array.isArray(customAnswers[q.id]) && (customAnswers[q.id] as unknown[]).length > 0 && typeof (customAnswers[q.id] as unknown[])[0] === "object")
      ? (customAnswers[q.id] as FileAnswer[])
      : [];
    const incoming = Array.from(files);
    if (existing.length + incoming.length > maxFiles) {
      toast.error(`You can upload up to ${maxFiles} file${maxFiles === 1 ? "" : "s"} for "${q.label}".`);
      return;
    }
    for (const f of incoming) {
      if (f.size > ATTENDEE_FILE_MAX_SIZE) {
        toast.error(`"${f.name}" is larger than 90 MB.`);
        return;
      }
    }
    setUploadingQuestionId(q.id);
    try {
      const uploaded: FileAnswer[] = [];
      for (const f of incoming) {
        const safeName = f.name.replace(/[^a-zA-Z0-9._-]/g, "_");
        const path = `${eventSlug}/${q.id}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${safeName}`;
        const { error: upErr } = await supabase.storage.from("attendee-uploads").upload(path, f, { upsert: false, contentType: f.type || undefined });
        if (upErr) throw upErr;
        const { data: urlData } = supabase.storage.from("attendee-uploads").getPublicUrl(path);
        uploaded.push({ name: f.name, url: urlData.publicUrl, size: f.size, type: f.type });
      }
      updateCustomAnswer(q.id, [...existing, ...uploaded]);
  ...
  const removeUploadedFile = (q: CustomQuestion, idx: number) => {
    const current = (customAnswers[q.id] as FileAnswer[]) || [];
    updateCustomAnswer(q.id, current.filter((_, i) => i !== idx));
  };
  ...
  const validateCustomQuestions = (chunk?: CustomQuestion[]) => {
    ...
    for (const q of qs) {
      if (uploadingQuestionId === q.id) {
        toast.error(`Please wait for "${q.label}" upload to finish.`); return false;
      }
      if (q.required) {
    ...
    <div className="space-y-4">
      ...
      {chunk.map(q => (
        <div key={q.id} className="space-y-1.5">
          ...
          {q.type === "file" && (() => {
            const maxFiles = Math.max(1, Math.min(10, q.maxFiles || 1));
            const uploaded = (Array.isArray(customAnswers[q.id]) && (customAnswers[q.id] as unknown[]).length > 0 && typeof (customAnswers[q.id] as unknown[])[0] === "object")
              ? (customAnswers[q.id] as FileAnswer[])
              : [];
            const canAddMore = uploaded.length < maxFiles;
            const isUploading = uploadingQuestionId === q.id;
            return (
              <div className="space-y-2">
                <p className="text-[11px] text-muted-foreground">
                  Upload up to {maxFiles} file{maxFiles === 1 ? "" : "s"} (photos, videos, or documents). Max 90 MB each.
                </p>
                {uploaded.length > 0 && (
                  <div className="space-y-1.5">
                    {uploaded.map((f, i) => (
                      <div key={i} className="flex items-center justify-between gap-2 rounded-md border border-primary/20 bg-primary/5 p-2 text-xs">
                        <a href={f.url} target="_blank" rel="noopener noreferrer" className="truncate text-foreground hover:underline">
                          {f.name}
                        </a>
                        <button type="button" onClick={() => removeUploadedFile(q, i)} className="text-destructive shrink-0 text-[11px] hover:underline">
                          Remove
                        </button>
                      </div>
                    ))}
                ...
                {canAddMore && (
                  <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-border bg-card p-3 text-xs text-muted-foreground hover:border-primary transition-colors">
                    {isUploading ? <Loader2 className="h-4 w-4 animate-spin text-primary" /> : <Upload className="h-4 w-4" />}
                    <span>{isUploading ? "Uploading..." : `Click to upload (${uploaded.length}/${maxFiles})`}</span>
                    <input
                      type="file"
                      className="hidden"
                      multiple={maxFiles > 1}
                      ...
                      onChange={e => {
                        handleFileUploadForQuestion(q, e.target.files);
                        e.target.value = "";
                      }}
                    />
                  </label>
